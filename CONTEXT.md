...
**指引词（guidance word）**:
点一下即可复制、粘给任意 Agent 接着干的完整指令文。出厂是**内置段**（逐字固定）；使用者可为某一面撰写**自定义段**，非空时整段取代内置段，为空即回落内置段——中英各判各的。可改写的是五面：grill / spec / tickets / implement 四个阶段格，加票行；下一步卡的按钮在未完工时与阶段格同源，完工后换另一段固定收尾文案，空态页的工作约定与建骨架指令同样固定。**票行那面是唯一带槽的一面**（`{key}` `{path}` `{title}`）——它的内置段本身是「是哪张票」的身份证明，自定义段同样可用这三个槽；其余四面无槽。内置段不预设任何工程习惯，也不点名 Matt 技能包以外的技能。
