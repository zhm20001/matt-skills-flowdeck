...
**设置弹窗（settings modal）**:
网页里集中修改 config.json 可改字段的入口：pollMs 保存即生效，host/port 写入后重启生效，访问令牌只写不读。指引词分区也在这里——当前接通 implement 一面的自定义段编辑（其余四面在同 effort 的票 03 接上）。追踪目录与常用目录不入内（顶栏已有专门交互）。
...
**指引词（guidance word）**:
点一下即可复制、粘给任意 Agent 接着干的完整指令文。出厂是**内置段**（逐字固定）；使用者可为某一面撰写**自定义段**，非空时整段取代内置段，为空即回落内置段——中英各判各的。自定义段存在 `config.json` 的 `guides` 字段里（`guides.implement.zh` 这样），一个字段装全部面，编辑入口在设置弹窗「指引词」分区；服务端只把原值搬进 `/api/state`、不参与拼装，取舍全在复制那一刻的界面侧。面名不写死，写进去的面原样存着，但**当前只有 implement 一面会被复制取用**——其余四面在同一 effort 的票 03 接上，先写进去的值不会丢。可改写的是五面：grill / spec / tickets / implement 四个阶段格，加票行；下一步卡的按钮在未完工时与阶段格同源，完工后换另一段固定收尾文案，空态页的工作约定与建骨架指令同样固定。**票行那面是唯一带槽的一面**（`{key}` `{path}` `{title}`）——它的内置段本身是「是哪张票」的身份证明，自定义段同样可用这三个槽；其余四面无槽。内置段不预设任何工程习惯，也不点名 Matt 技能包以外的技能。
