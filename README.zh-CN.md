...
- 当前阶段的「下一步」指引词，点一下即复制，粘给任意 Agent 就能接着干；流程链标题行右上角常驻一个小「？」，点开是一篇**按四个阶段串起各自技能**（grilling / wayfinder / to-spec / to-tickets / implement）的导读——「哪个阶段背后是什么技能」一篇看完；
...
- **挂起的那张卡完全停下来**：不在前台的项目零定时器、零请求、零桌面通知——不管你把轮询模式选了哪一档。代价是它停在离开时的样子，所以卡面上摆着它离开那一刻的「最近刷新」时刻（数据被诚实地标为旧数据，不伪装成新的）；切回去先看到那份旧内容、一条「已恢复刷新」的轻提示，随后立刻补上最新的一拍。当前活跃那张则完全按你选的轮询模式行事（观测：可见才轮询、不可见停表、回前台补一拍；展示：常轮；惰性：只手动），标签条不改动这层语义。切回时若那个目录已被删除，换根失败沿用既有提示并留在原来那张卡上，不会被静默吞掉。把卡**全关**掉更进一步——那是你明说「我不想它在动了」，连手动「立即刷新」也一并停手（按钮停用并讲出原因，空态也说明这件事；开一张标签即收回）；这个表态只管本次会话，重开页面时服务端追踪目录仍是真相，标签照旧补位。
