...
界面随视口**流式缩放**：宽屏时字号、留白与主容器同比舒展（宽度 `1600px` 封顶、守住行长），小屏落回改造前的尺寸；三套平级主题（默认冷白，另有暖纸与 GitHub 暗）在右上角下拉切换，偏好都记在本浏览器。
...
| `styles/app.css` | 界面全部业务样式（原 index.html 内联 `<style>` 抽出）；开头的 `--fd-*` 别名块是 tokens 的唯一消费层，业务样式零裸值；流式缩放基座也住这里——`html { font-size: calc(var(--fluid-base) * var(--ui-scale)) }`（`--fluid-base: clamp(10px, 0.5vw + 4.6px, 11.5px)`），字号与 padding/margin/gap 全按 `1rem = 10px` 走 rem，组件固定宽度（菜单/弹窗）、圆角、边框留 px；主容器 `max-width: clamp(1080px, 92vw, 1600px)`——宽屏顶得满、行长有上限，各弹窗自定宽度不跟容器一起胀 |
| `styles/tokens-cold.css` | 冷白主题 tokens（**默认主题**，`data-theme="cold"`）：paper-palette-lab 生成件逐字节拷贝、仅 re-scope，保留自身色值；色值/字体栈只允许出现在 tokens 文件里 |
