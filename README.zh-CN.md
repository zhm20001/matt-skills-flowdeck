...
4. 实现（implement）：在为本次 effort 开的新分支上逐票实现（未开则先 `git switch -c <特性名>-impl`）；每完成一张票，就把该票文件里的 Status 行改为 resolved；全部票关闭并验证通过后合回 main。
```
