...
| `styles/app.css` | all business styles for the UI (extracted from index.html's former inline `<style>`); its leading `--fd-*` alias block is the single consumption layer over the tokens, and business styles carry zero raw values |
| `styles/tokens-cold.css` | cold-white theme tokens (**the default**, `data-theme="cold"`): a byte-copy of the paper-palette-lab output, re-scoped only, keeping its own colors; colors and font stacks live only in tokens files |
