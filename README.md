...
- **Agent-agnostic**: Claude Code, Cursor, or files you write by hand — flowdeck tracks passively. Any agent that follows the file conventions below and writes into `.scratch/` shows up in the browser:
  ...
  - a **Guidance words** section in the settings modal to rewrite the Implement stage's guidance wholesale: one box per language, plus a read-only preview of what you'd get after saving and a "Restore default" button. Anything you write replaces the built-in text wholesale (it is not prepended); empty falls back to the built-in, and each language is judged on its own. It lives in the `guides` field of `config.json`, so it travels with the project rather than staying in this browser — and the factory text names no git instruction and no skill outside Matt's pack.
