...
4. Implementation: work the tickets one at a time on a new branch created for the effort
   (create one first if none yet: `git switch -c <feature-slug>-impl`); as each ticket finishes,
   change its Status line to resolved; once all tickets are closed and verified, merge back to main.
```
